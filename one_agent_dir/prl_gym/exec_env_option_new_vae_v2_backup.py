...
class ExecEnv1(ExecEnv):
    ...
    def __init__(self, config, program_seq=None, metadata={}):
        ...
        self.init_state = self.init_states[0][0]  #self.cnn_input_states[0]
        self.reset_count = 0
    ...
    def execute_pred_program(self, program_seq, demo=None, demo_len=None):
        ...
        pred_program['current_state'] = final_state #self.cnn_input_states[self.program_step]
...
class ExecEnv_option(ExecEnv):
    ...
    def __init__(self, config, metadata={}):
        ...
        self._world.set_new_state(self.current_state, self.metadata)
    ...
    def execute_pred_program(self, program_seq):
        ...
        if not s_exe or not len(program_seq) > 4:
            ...
            pred_program['s_h'] = demos_s_h
            pred_program['a_h'] = demos_a_h
            pred_program['reward_h'] = np.zeros([1], dtype=np.float32) - 0.1 # penalty for invalid programs
        ...
        pred_program['program'] = program_seq
        pred_program['current_state'] = self.current_state
        ...
        #    pred_program['mean_reward'] = sum([pred_program['reward_h'][i][x-1] if x-1>0 else 0 for i,x in enumerate(pred_program['a_h_len'])]) / 1
        #    assert len(pred_program['a_h_len']) == 1
        ...
        #else:
        #    pred_program['mean_reward'] = np.sum(pred_program['reward_h']) / 1
